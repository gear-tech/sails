...
impl<'a> RootGenerator<'a> {
    fn new(service_name: &'a str) -> Self {
        ...
        code.push_str("use sails_rtl::{prelude::*, String, calls::RemotingAction, gstd::calls::Remoting, gstd::calls::Args};\n");
        code.push_str("#[allow(unused_imports)]\n");
...
impl<'ast> Visitor<'ast> for CtorTraitGenerator {
    fn visit_ctor(&mut self, ctor: &'ast Ctor) {
        self.code
            .push_str(&format!("pub trait {}Constructors {{\n", self.service_name));
    ...
    fn visit_ctor_func(&mut self, func: &'ast CtorFunc) {
        ...
        self.code
            .push_str(&format!("fn {fn_name}(remoting: Remoting,"));

        visitor::accept_ctor_func(func, self);

        self.code
            .push_str(")-> RemotingAction<Remoting, Args, ActorId>;\n");
    }
...
impl<'ast> Visitor<'ast> for CtorFactoryGenerator {
    fn visit_ctor(&mut self, ctor: &'ast Ctor) {
        ...
        self.code.push_str(&format!(
            "impl {}Constructors for ProgramFactory {{\n",
            self.service_name,
        ));
        visitor::accept_ctor(ctor, self);
        self.code.push_str("}\n");
    ...
    fn visit_ctor_func(&mut self, func: &'ast CtorFunc) {
        ...
        self.code
            .push_str(&format!("fn {fn_name_snake}(remoting: Remoting,",));
...
impl<'ast> Visitor<'ast> for ServiceGenerator {
    ...
    fn visit_func_output(&mut self, func_output: &'ast TypeDecl) {
        ...
        self.code.push_str(&format!(
            ") ->  RemotingAction<Remoting, TCallArgs, {}>",
            type_decl_code
        ));
    }
}
...
impl<'ast> Visitor<'ast> for ClientGenerator {
    fn visit_service(&mut self, service: &'ast Service) {
        ...
        self.code.push_str(&format!(
            ...
            impl<A: Default> {name}<A> for {name}Client {{
        "#
        ));
...
mod tests {
    ...
    fn test_basic_works() {
        ...
        insta::assert_snapshot!(generate(program, "Service").unwrap());
    }
