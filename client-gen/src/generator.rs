...
impl IdlGenerator {
    ...
    pub fn generate(self, program: Program) -> Result<String> {
        let mut trait_generator = RootGenerator::new("Service", &self.path);
...
impl<'a> RootGenerator<'a> {
    fn new(service_name: &'a str, idl_path: &'a Path) -> Self {
        let mut code = String::new();

        code.push_str("// Code generated by sails-client-gen from ");
        code.push_str(idl_path.file_name().unwrap().to_string_lossy().as_ref());
        code.push_str(". DO NOT EDIT.\n");

        code.push_str("#![automatically_derived]\n");
...
impl<'a, 'ast> Visitor<'ast> for RootGenerator<'a> {
    ...
    fn visit_type(&mut self, t: &'ast Type) {
        let mut type_generator = TypeGenerator::new(t.name());
        type_generator.visit_type(t);
        self.code.push_str(&type_generator.code);
    }
}

#[derive(Default)]
struct RequestStructsGenerator {
    code: String,
}

impl<'ast> Visitor<'ast> for RequestStructsGenerator {
    fn visit_func(&mut self, func: &'ast Func) {
        ...
        self.code
            .push_str(&format!("struct {}RequestArgs {{\n", method_name));
