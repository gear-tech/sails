...
import { DemoClient } from './demo-v2/lib';
...
describe('Ping', () => {
  let program: DemoClient;

  test('create program', async () => {
    program = new DemoClient(api);
...
describe('Counter', () => {
  ...
  test('add', async () => {
    ...
    const unsub = await (program.counter as any).subscribeToAddedEvent((data: number) => {
      addedEventData = data;
    });
  ...
  test('sub', async () => {
    ...
    const unsub = await (program.counter as any).subscribeToSubtractedEvent((data: number) => {
