...
use scale_info::{
    form::PortableForm, PortableRegistry, Type, TypeDef, TypeDefPrimitive, TypeDefTuple,
};
...
fn primitive_type_name(type_def: &TypeDefPrimitive) -> Result<String> {
    match type_def {
        ...
        TypeDefPrimitive::Char => Ok("char".into()), // Do we want to support it? If such it will require a definition
        TypeDefPrimitive::Str => Ok("string".into()),
...
mod tests {
    ...
    fn generic_struct_type_name_resolution_works() {
        ...
        assert_eq!(
            string_struct_name,
            "SailsIdlgenTypeNamesTestsGenericStruct<string>"
        );
    }
    ...
    fn generic_variant_type_name_resolution_works() {
        ...
        assert_eq!(
            ...
            "SailsIdlgenTypeNamesTestsGenericEnum<u32, string>"
        );
    ...
    fn result_type_name_resolution_works() {
        ...
        assert_eq!(result_name, "Result<u32, string>");
    }

    #[test]
    fn option_type_name_resolution_works() {
        let mut registry = Registry::new();
        let option_id = registry.register_type(&MetaType::new::<Option<u32>>()).id;
        let portable_registry = PortableRegistry::from(registry);

        let type_names = resolve_type_names(&portable_registry).unwrap();

        let option_name = type_names.get(&option_id).unwrap();
        assert_eq!(option_name, "Option<u32>");
    }

    #[test]
    fn tuple_type_name_resolution_works() {
        let mut registry = Registry::new();
        let tuple_id = registry
            .register_type(&MetaType::new::<(u32, String, bool)>())
            .id;
        let portable_registry = PortableRegistry::from(registry);

        let type_names = resolve_type_names(&portable_registry).unwrap();

        let tuple_name = type_names.get(&tuple_id).unwrap();
        assert_eq!(tuple_name, "struct { u32, string, bool }");
    }

    #[test]
    fn array_type_name_resolution_works() {
        let mut registry = Registry::new();
        let array_id = registry.register_type(&MetaType::new::<Vec<u32>>()).id;
        let portable_registry = PortableRegistry::from(registry);

        let type_names = resolve_type_names(&portable_registry).unwrap();

        let array_name = type_names.get(&array_id).unwrap();
        assert_eq!(array_name, "Vec<u32>");
    }
}
