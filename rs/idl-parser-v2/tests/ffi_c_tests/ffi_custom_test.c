...
int program_service_item_visited = 0;
...
void c_visit_program_service_item(const void *context,
  ...
  printf("C: visit_program_service_item called\n");
...
int main() {
  ...
  Visitor my_visitor = {
      ...
      .visit_service_unit = (void (*)(const void *, const ServiceUnit *))
          unexpected_ffi_call,  // Should not be called
      .visit_program_service_item =
          (void (*)(const void *, const ProgramServiceItem *))
              unexpected_ffi_call,  // Should not be called
  ...
  assert(program_service_item_visited == 0 &&
         "visit_program_service_item should NOT have been called");
