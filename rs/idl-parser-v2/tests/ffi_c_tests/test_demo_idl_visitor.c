...
static int c_visit_program_service_item_calls = 0;
...
void c_visit_program_service_item(const void *context,
                                  const ProgramServiceItem *service_item) {
  (void)context;
  (void)service_item;
  c_visit_program_service_item_calls++;
}
...
int main() {
  ...
  Visitor partial_visitor = {
      ...
      .visit_option_type_decl = NULL,
      .visit_result_type_decl = NULL,
      .visit_primitive_type = NULL,
      .visit_user_defined_type = NULL,
  ...
  assert(c_visit_service_unit_calls == 6 &&
         ...
         "services");
  assert(c_visit_program_service_item_calls == 6 &&
         "c_visit_program_service_item should have been called 6 times");
