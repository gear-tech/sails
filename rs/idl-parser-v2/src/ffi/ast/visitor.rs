...
unsafe extern "C" {
    ...
    fn visit_alias_def(context: *const (), alias_def: *const AliasDef);
...
impl<'a, 'ast> RawVisitor<'ast> for VisitorWrapper<'a> {
    ...
    fn visit_alias_def(&mut self, alias_def: &'ast ast::AliasDef) {
        if let Some(visit) = self.visitor.visit_alias_def {
            let ffi_alias = AliasDef::from_ast(alias_def, &mut self.allocations);
            unsafe { visit(self.context, &ffi_alias) };
            return;
        }
        crate::visitor::accept_alias_def(alias_def, self);
    }
