...
pub unsafe extern "C" fn parse_idl(source_ptr: *const c_char) -> *mut ParseResult {
    ...
    // Parse AST
    let ast_doc = match ast::IdlDoc::parse(source) {
        Ok(doc) => doc,
        Err(e) => {
...
impl ServiceFunc {
    pub fn from_ast(service_func: &ast::ServiceFunc, allocations: &mut Allocations) -> Self {
        ...
        ServiceFunc {
            ...
            name_ptr: name_ffi.ptr,
            name_len: name_ffi.len,
            is_query: service_func.is_query,
            docs_ptr,
            docs_len,
            annotations_ptr,
            annotations_len,
...
/// FFI-safe representation of a [crate::ast::ProgramServiceItem].
#[repr(C)]
...
impl ProgramServiceItem {
    pub fn from_ast(
        program_service_item: &ast::ProgramServiceItem,
        allocations: &mut Allocations,
    ) -> Self {
        let name_ffi = allocate_string(&program_service_item.name, allocations);
        let route_ffi = allocate_string(&program_service_item.route, allocations);
        let (docs_ptr, docs_len) = allocate_ffi_string_vec(&program_service_item.docs, allocations);
        let (annotations_ptr, annotations_len) =
            allocate_annotation_vec(&program_service_item.annotations, allocations);

        ProgramServiceItem {
            raw_ptr: Ptr::from(program_service_item),
            name_ptr: name_ffi.ptr,
            name_len: name_ffi.len,
            route_ptr: route_ffi.ptr,
            route_len: route_ffi.len,
            docs_ptr,
            docs_len,
            annotations_ptr,
            annotations_len,
