...
use ethapp_sol_client::IEthappClient;
use sails_rs::{
    ActorId,
...
async fn ethapp_sol_generated_client_payloads_work() {
    ...
    let payload = client.create_prg((false,).abi_encode_sequence()).encode_call();
    ...
    let payload = client
        .svc_1_do_this((false, 42u32, "hello").abi_encode_sequence())
        .encode_call();
...
async fn ethapp_remoting_encode_reply_with_generated_client_payloads_works() {
    ...
    let payload = client.create_prg((true,).abi_encode_sequence()).encode_call();
    ...
    let reply_payload = client
        .svc_1_do_this((true, 42u32, "hello").abi_encode_sequence())
        .send_for_reply()
        .expect("send for reply")
        .await
        .expect("reply payload");
...
async fn ethapp_ctor_payable_works_with_value_generated_client_payloads() {
    ...
    let payload = client
        .create_payable((false,).abi_encode_sequence())
        .encode_call();
...
async fn ethapp_method_payable_works_with_value_generated_client_payloads() {
    ...
    let reply_payload = client
        .svc_1_do_this_payable((false, 42u32).abi_encode_sequence())
        .with_params(|params| params.with_value(1000))
        .send_for_reply()
        .expect("send for reply")
        .await
