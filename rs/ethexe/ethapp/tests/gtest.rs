...
#[tokio::test]
async fn ethapp_override_different_name_works() {
    let system = System::new();
    system.init_logger_with_default_filter("gwasm=debug,gtest=debug,sails_rs=debug");
    system.mint_to(ADMIN_ID, 1_000_000_000_000_000);
    ...
    // The 'inherited' route has two methods in its Solidity interface:
    // 1. 'inheritedFoo' (inherited from BaseService)
    // 2. 'inheritedBar' (its own method)
    
    // We call 'inheritedFoo' (the base method)
    let sig = sails_rs::solidity::selector("inheritedFoo(bool)");
    let params = (false,).abi_encode_sequence();
    let payload = [sig.as_slice(), params.as_slice()].concat();
    ...
    let reply = u32::abi_decode(reply_log_record.payload());
    
    // It should return 200, because 'bar' overrides 'foo'!
    assert_eq!(reply, Ok(200), "Calling inheritedFoo should trigger the 'bar' override and return 200");

    // Also verify that 'inheritedBar' works directly
    let sig_bar = sails_rs::solidity::selector("inheritedBar(bool)");
    let payload_bar = [sig_bar.as_slice(), params.as_slice()].concat();
    let message_id = program.send_bytes(ADMIN_ID, payload_bar);
    let run_result = system.run_next_block();
    let reply_log_record = run_result
        .log()
        .iter()
    ...
    let reply = u32::abi_decode(reply_log_record.payload());
    assert_eq!(reply, Ok(200), "Calling inheritedBar directly should also return 200");
}
