...
enum SailsCommands {
    ...
    /// Generate client code from IDL in Rust
    ...
    /// Generate client code from IDL in C#
    #[command(name = "client-net")]
    ClientNet {
        /// Path to the IDL file
        #[arg(value_hint = clap::ValueHint::FilePath)]
        idl_path: PathBuf,
        /// Path to the output C# client file
        #[arg(value_hint = clap::ValueHint::FilePath)]
        out_path: Option<PathBuf>,
        /// Generate code in specified namespace
        #[arg(long)]
...
fn main() -> Result<(), i32> {
    ...
    let result = match command {
        ...
        SailsCommands::ClientRs {
            ...
            let mut client_gen =
                sails_client_gen::ClientGenerator::from_idl_path(idl_path.as_ref());
        ...
        SailsCommands::ClientNet {
            ...
            let out_path = out_path.unwrap_or_else(|| idl_path.with_extension("g.cs"));
            client_gen.generate_to(out_path)
        }
    };
