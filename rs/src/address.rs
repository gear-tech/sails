use crate::{
    ...
    prelude::*,
    type_info::{Registry, Type, TypeBuilder, TypeDecl},
};
...
impl TypeInfo for Address {
    ...
    fn type_decl(_registry: &mut Registry) -> TypeDecl {
        TypeDecl::named("Address".into())
    }

    fn type_def(registry: &mut Registry) -> Option<Type> {
        let h160_decl = H160::type_decl(registry);
        Some(
            TypeBuilder::new()
                .name("Address")
                .annotate("sol-type")
...
mod tests {
    ...
    fn type_info_has_soltype_address_annotation() {
        ...
        let type_ref = registry.register_type::<Address>();
        let ty = registry.get_type(type_ref).unwrap();
        assert_eq!(ty.name, "Address");
        assert!(
            ty.annotations
                .iter()
                .any(|(name, value)| name == "sol-type" && value.as_deref() == Some("address")),
