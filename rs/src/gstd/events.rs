...
use crate::{ValueUnit, Vec, errors::*};
...
struct SysCalls {
    #[allow(dead_code)] // TODO: remove this code w/o wasm32 target arch
    ...
    TEvents: parity_scale_codec::Encode + scale_info::StaticTypeInfo,
{
    if let Some(sys_calls) = SysCalls::as_ref() {
        let payload = codec::compose_payload::<TEvents>(
            crate::gstd::services::exposure_context((sys_calls.msg_id)()).route(),
            event,
        )?;
        (sys_calls.msg_send_bytes)(gstd::ActorId::zero(), payload, 0)?;
    } else {
        codec::compose_payload::<TEvents>(&[], event)?;
    }
    Ok(())
}
