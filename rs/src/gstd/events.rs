...
/// Encodes an event with a prefix and returns the encoded bytes using a closure.
#[allow(dead_code)]
...
pub trait ServiceWithEvents: super::services::Service {
    type Events: SailsEvent;

    fn emitter() -> EventEmitter<Self::Events> {
        use super::services::Exposure;

        #[cfg(target_arch = "wasm32")]
        let route = Self::Exposure::scoped_route().expect("Route must be set for the event");
        // This is a workaround, where the route is not set in the static context.
        #[cfg(not(target_arch = "wasm32"))]
        let route = Self::Exposure::scoped_route().unwrap_or_default();
        EventEmitter::new(route)
    }
}

pub trait ExposureWithEvents: super::services::Exposure {
    type Events: SailsEvent;
