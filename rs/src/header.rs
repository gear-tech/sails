...
impl SailsMessageHeaderBuilder {
    ...
    /// Sets the version for the header
    ///
    /// If version is 1, then reserved byte is set to 0 automatically.
    ///
    /// # Panics
    /// If version is invalid, then panics. For non-panicking version of the method use [`SailsMessageHeaderBuilder::version_checked`].
    /// The version is invalid if:
    /// - version is 0
    /// - version is greater than highest supported version
    pub fn version(mut self, version: u8) -> Self {
        let version = Version::new(version).expect("invalid version provided");
    ...
    pub fn version_checked(mut self, version: u8) -> Result<Self, &'static str> {
    ...
    /// Sets the route ID for the header.
    pub fn route_id(mut self, route_id: u8) -> Self {
        self.route_id = route_id;
        self
    }

    /// Builds the Sails message header.
    ///
    /// If version is not set, uses the latest supported version.
    pub fn build(self) -> SailsMessageHeader {
        SailsMessageHeader {
            version: self.version.unwrap_or_else(Version::latest),
