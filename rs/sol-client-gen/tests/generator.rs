...
service Svc1 {
    functions {
        DoThis(p1: bool, p2: u32, p3: string) -> u32;
    }
}
...
#[test]
fn test_sol_client_generation_basic() {
    let code = ClientGenerator::from_idl(SOL_IDL)
        .with_sails_crate("sails_rs")
        .generate()
        .expect("generate sol client");

    assert!(code.contains("io_struct_sol_impl!(CreatePrg"));
    assert!(code.contains("io_struct_sol_impl!("));
    assert!(code.contains("selector_prefix"));
