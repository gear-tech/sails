...
impl<'ast> Visitor<'ast> for ServiceGenerator<'ast> {
    ...
    fn visit_service_func(&mut self, func: &'ast ast::ServiceFunc) {
        ...
        let service_route = self.service_route.to_case(Case::Camel);

        quote_in! { self.io_tokens =>
            io_struct_sol_impl!(
                $fn_name ($params_with_types_super) -> $output_type_decl_code,
                $entry_id,
                <super::$(self.service_name)Impl as $(self.sails_path)::client::Identifiable>::INTERFACE_ID,
                $(quoted(service_route.as_str())),
                $(quoted(func.name.as_str()))
            );
        };
    }
}
