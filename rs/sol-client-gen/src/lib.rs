...
impl<'ast, S> ClientGenerator<'ast, S> {
    ...
    /// Add a map from IDL type to crate path.
    ///
    ...
    /// ```
    /// let code = sails_sol_client_gen::ClientGenerator::from_idl("<idl>")
    ...
    /// Derive only necessary [`parity_scale_codec::Encode`], [`parity_scale_codec::Decode`], [`scale_info::TypeInfo`] and [`sails_reflect_hash::ReflectHash`] traits for generated types.
    ///
