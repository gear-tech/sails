...
impl<'ast> Visitor<'ast> for CtorGenerator<'ast> {
    ...
    fn visit_ctor_func(&mut self, func: &'ast ast::CtorFunc) {
        ...
        let ctor_name = func.name.to_case(Case::Camel);
        quote_in! { self.io_tokens =>
            io_struct_sol_impl!($fn_name ($params_with_types_super) -> (), $entry_id, $(self.sails_path)::meta::InterfaceId::zero(), "", $(quoted(ctor_name.as_str())));
        };
    }
}
