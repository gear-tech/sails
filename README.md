...
impl MyCounter {
    ...
    fn emit_event(&mut self, event: MyCounterEvent) -> Result<()> {
        ...
    }
}
```
...
During message processing, `Sails` program can can obtain details of incoming messages and current execution environment by using `Syscall` struct which provides a collection of methods that abstract lower-level operations ([`message_source`],  [`message_size`], [`message_id`], [`message_value`], [`program_id`], etc.).
